  ...
  static async encrypt(
    ...
    if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle) {
      try {
        ...
        if (associatedData) {
          params.additionalData = associatedData;
        }
      ...
      try {
        const crypto: typeof import('crypto') = await import('crypto');
        ...
        if (associatedData) {
          (cipher as crypto.CipherGCM).setAAD(Buffer.from(associatedData));
        }
    ...
    if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle) {
      ...
      try {
        ...
        const decipher = crypto.createDecipheriv(
          `aes-${key.length * 8}-gcm`, 
          Buffer.from(key), 
          Buffer.from(encResult.iv)
        );
        
        (decipher as crypto.DecipherGCM).setAuthTag(Buffer.from(encResult.authTag));
        
        if (associatedData) {
          (decipher as crypto.DecipherGCM).setAAD(Buffer.from(associatedData));
        }
